...
void world::plateBoundaryCondition()
{
	for (int i = 0; i < 9; i++)
	{
		plate->setVertexBoundaryCondition(plate->getVertex(indexArray(i)), indexArray(i));
	}
	
	//plate->setVertexBoundaryCondition(plate->getVertex(162), 162);
	//plate->setVertexBoundaryCondition(plate->getVertex(200), 200);
	//plate->setVertexBoundaryCondition(plate->getVertex(238), 238);
	//plate->setVertexBoundaryCondition(plate->getVertex(276), 276);

	plate->setThetaBoundaryCondition(plate->getTheta(0), 0);
	plate->setThetaBoundaryCondition(plate->getTheta(1), 1);
	plate->setThetaBoundaryCondition(plate->getTheta(2), 2);
	plate->setThetaBoundaryCondition(plate->getTheta(3), 3);

	Vector3d xEnd = plate->getVertex(plate->nv - 1);
	plate->setOneVertexBoundaryCondition(xEnd(0), plate->nv - 1 , 0);
	plate->setOneVertexBoundaryCondition(xEnd(1), plate->nv - 1 , 1);
}
...
void world::updateEachStep()
{
	if (currentTime < 0.45)
	{
		for (int i = 160; i < 308; i++)
		{
			plate->v_edgeElement[i].refLength = plate->v_edgeElement[i].refLength + 0.000001;
		}
	}
	...
	if (currentTime > 0.5 && currentTime < 1.5)
	{
		for (int i = 0; i < 9; i++)
		{
			...
			xCurrent(2) = xCurrent(2) + 0.0001;
			plate->setVertexBoundaryCondition(xCurrent, indexArray(i));
		}
	}
