		...
		double m_Possion, double m_dt)
{
	...
	GJ = YoungM/(2.0*(1.0+Possion)) * radius * radius * radius / 3;
...
void elasticPlate::setupMass()
{
	...
	massArray(3 * (nv-1) + 0) = 0.1;
	massArray(3 * (nv-1) + 1) = 0.1;
	massArray(3 * (nv-1) + 2) = 0.1;
...
void elasticPlate::setupGeometry()
{
	...
	coupleEdge.clear();
	ifstream inFile4;
	inFile4.open("inputdata/coupleEdge.txt");
	int dd, ee;
	while(inFile4 >> dd >> ee)
	{
		Vector2i edgeCurrent;
    	...
    	coupleEdge.push_back(edgeCurrent);
	}
	inFile4.close();
	...
	coupleBend.clear();
	ifstream inFile5;
	inFile5.open("inputdata/coupleBend.txt");
	int ff, gg;
	while(inFile5 >> ff >> gg)
	{
		Vector2i bendingCurrent;
    	...
    	coupleBend.push_back(bendingCurrent);
	}
	inFile5.close();
}
...
void elasticPlate::computeEdge()
{
	...
	for (int i = 0; i < edge.size(); i++)
	{
		...
		m_edgeElement.refLength0 = m_edgeElement.refLength;
	...
	v_coupleStretching.clear();

	v_coupleAngle.clear();

	for (int i = 0; i < coupleEdge.size(); i++)
	{
		Vector2i edgeCurrent = coupleEdge[i];
		...
		v_coupleStretching.push_back(m_edgeElement);

		
		angleElement m_angleElement;

		m_angleElement.nv_1 = m_edgeElement.nv_1 - 1;
		m_angleElement.nv_2 = m_edgeElement.nv_1;
		m_angleElement.nv_3 = m_edgeElement.nv_1 + 1;
		m_angleElement.nv_4 = m_edgeElement.nv_2;

		m_angleElement.arrayNum = VectorXi::Zero(12);

		m_angleElement.arrayNum(0) = 3 * m_angleElement.nv_1 + 0;
		m_angleElement.arrayNum(1) = 3 * m_angleElement.nv_1 + 1;
		m_angleElement.arrayNum(2) = 3 * m_angleElement.nv_1 + 2;
		
		m_angleElement.arrayNum(3) = 3 * m_angleElement.nv_2 + 0;
		m_angleElement.arrayNum(4) = 3 * m_angleElement.nv_2 + 1;
		m_angleElement.arrayNum(5) = 3 * m_angleElement.nv_2 + 2;

		m_angleElement.arrayNum(6) = 3 * m_angleElement.nv_3 + 0;
		m_angleElement.arrayNum(7) = 3 * m_angleElement.nv_3 + 1;
		m_angleElement.arrayNum(8) = 3 * m_angleElement.nv_3 + 2;

		m_angleElement.arrayNum(9)  = 3 * m_angleElement.nv_4 + 0;
		m_angleElement.arrayNum(10) = 3 * m_angleElement.nv_4 + 1;
		m_angleElement.arrayNum(11) = 3 * m_angleElement.nv_4 + 2;
		
		v_coupleAngle.push_back(m_angleElement);
...
void elasticPlate::computeBending()
{
	...
	for (int i = 0; i < coupleBend.size(); i++)
	{
		Vector2i bendingCurrent = coupleBend[i];

		bendingElementNew m_bendingElement;
		...
		m_bendingElement.e_1 = m_bendingElement.x_2 - m_bendingElement.x_1;
		m_bendingElement.e_2 = m_bendingElement.x_4 - m_bendingElement.x_3;
		...
		// dof
		m_bendingElement.arrayNum = VectorXi::Zero(14);
		...
		m_bendingElement.arrayNum(7) = 3 * m_bendingElement.nv_3 + 0;
		m_bendingElement.arrayNum(8) = 3 * m_bendingElement.nv_3 + 1;
		m_bendingElement.arrayNum(9) = 3 * m_bendingElement.nv_3 + 2;

		m_bendingElement.arrayNum(10) = 3 * nv + ne_2;
		
		m_bendingElement.arrayNum(11) = 3 * m_bendingElement.nv_4 + 0;
		m_bendingElement.arrayNum(12) = 3 * m_bendingElement.nv_4 + 1;
		m_bendingElement.arrayNum(13) = 3 * m_bendingElement.nv_4 + 2;

		//cout << m_bendingElement.arrayNum.transpose() << endl;

		v_coupleBending.push_back(m_bendingElement);
	}
}
...
void elasticPlate::updateEdgePair()
{
	for (int i = 0; i < edgeNum; i++)
	{
		...
		t1_old = v_edgeElement[i].t1;
		d1_old = v_edgeElement[i].d1;
		d2_old = v_edgeElement[i].d2;
		t1 = ( v_edgeElement[i].x_2 - v_edgeElement[i].x_1 ) / ( v_edgeElement[i].x_2 - v_edgeElement[i].x_1 ).norm();
		parallelTansport(d1_old, t1_old, t1, d1);
		d2 = t1.cross(d1);
		v_edgeElement[i].d1 = d1;
		v_edgeElement[i].d2 = d2;
	}


	for (int i = 0; i < v_coupleStretching.size(); i++)
	{
		v_coupleStretching[i].x_1 = getVertex(v_coupleStretching[i].nv_1);
		v_coupleStretching[i].x_2 = getVertex(v_coupleStretching[i].nv_2);
		v_coupleStretching[i].edgeLength = (v_coupleStretching[i].x_1 - v_coupleStretching[i].x_2).norm();
	}
}
...
void elasticPlate::updateBendingPair()
{
	...
	{
		v_coupleBending[i].x_1 = getVertex(v_coupleBending[i].nv_1);
		v_coupleBending[i].x_2 = getVertex(v_coupleBending[i].nv_2);
		v_coupleBending[i].x_3 = getVertex(v_coupleBending[i].nv_3);
		v_coupleBending[i].x_4 = getVertex(v_coupleBending[i].nv_4);

		v_coupleBending[i].theta_1 = getTheta(v_coupleBending[i].ne_1);
		v_coupleBending[i].theta_2 = getTheta(v_coupleBending[i].ne_2);

		if ( v_coupleBending[i].sign_1 < 0 )
        {
            v_coupleBending[i].theta_1 = - v_coupleBending[i].theta_1;
        }

        if ( v_coupleBending[i].sign_2 < 0 )
        {
            v_coupleBending[i].theta_2 = - v_coupleBending[i].theta_2;
        }

		v_coupleBending[i].e_1 = v_coupleBending[i].x_2 - v_coupleBending[i].x_1;
		v_coupleBending[i].e_2 = v_coupleBending[i].x_4 - v_coupleBending[i].x_3;

		v_coupleBending[i].norm_1 =  v_coupleBending[i].e_1.norm();
		v_coupleBending[i].norm_2 =  v_coupleBending[i].e_2.norm();

		v_coupleBending[i].t_1 = v_coupleBending[i].e_1 / v_coupleBending[i].norm_1;
		v_coupleBending[i].t_2 = v_coupleBending[i].e_2 / v_coupleBending[i].norm_2;

		// computeTimeParallel
		Vector3d d1, d2, t1;
		Vector3d d1_old, d2_old, t1_old;

		t1_old = v_coupleBending[i].t_1_old;
		d1_old = v_coupleBending[i].d_11_old;
		d2_old = v_coupleBending[i].d_12_old;
		t1 = v_coupleBending[i].t_1;
		parallelTansport(d1_old, t1_old, t1, d1);
		d2 = t1.cross(d1);
		v_coupleBending[i].d_11 = d1;
		v_coupleBending[i].d_12 = d2;

		t1_old = v_coupleBending[i].t_2_old;
		d1_old = v_coupleBending[i].d_21_old;
		d2_old = v_coupleBending[i].d_22_old;
		t1 = v_coupleBending[i].t_2;
		parallelTansport(d1_old, t1_old, t1, d1);
		d2 = t1.cross(d1);
		v_coupleBending[i].d_21 = d1;
		v_coupleBending[i].d_22 = d2;

		// refTwist
		Vector3d u_e, u_f, t_e, t_f, u_temp;

		u_e = v_coupleBending[i].d_11;
		u_f = v_coupleBending[i].d_21;
		t_e = v_coupleBending[i].t_1;
		t_f = v_coupleBending[i].t_2;
		parallelTansport(u_e, t_e, t_f, u_temp);
		rotateAxisAngle(u_temp, t_f, v_coupleBending[i].refTwist_old);
		double sgnAngle = signedAngle(u_temp, u_f, t_f);
        v_coupleBending[i].refTwist = v_coupleBending[i].refTwist_old + sgnAngle;

        // material director
        double cs, ss;

        cs = cos(v_coupleBending[i].theta_1);
        ss = sin(v_coupleBending[i].theta_1);
        v_coupleBending[i].m_11 =  cs * v_coupleBending[i].d_11 + ss * v_coupleBending[i].d_12;
        v_coupleBending[i].m_12 = -ss * v_coupleBending[i].d_11 + cs * v_coupleBending[i].d_12;

        cs = cos(v_coupleBending[i].theta_2);
        ss = sin(v_coupleBending[i].theta_2);
        v_coupleBending[i].m_21 =  cs * v_coupleBending[i].d_21 + ss * v_coupleBending[i].d_22;
        v_coupleBending[i].m_22 = -ss * v_coupleBending[i].d_21 + cs * v_coupleBending[i].d_22;

        // curvature
		v_coupleBending[i].kb = 2.0 * v_coupleBending[i].t_1.cross(v_coupleBending[i].t_2) / (1.0 + v_coupleBending[i].t_1.dot(v_coupleBending[i].t_2));
		v_coupleBending[i].kappa(0) = 0.5 * v_coupleBending[i].kb.dot(v_coupleBending[i].m_12 + v_coupleBending[i].m_22);
		v_coupleBending[i].kappa(1) =-0.5 * v_coupleBending[i].kb.dot(v_coupleBending[i].m_11 + v_coupleBending[i].m_21);
	}
