...
void elasticBendingForce::computeFb()
{
    ...
    for (int i = 0; i < plate->bendingNum; i++)
    {
        ...
        {
            localEI = rotateMatrix * EIMat * rotateMatrix.transpose();
        }

        f = - relevantPart * localEI * kappaL / plate->v_bendingElement[i].voroniLength;
...
void elasticBendingForce::computeJb()
{
	...
	for (int i = 0; i < plate->bendingNum; i++)
    {
        ...
        Jbb = - 1.0/len * relevantPart * localEI * relevantPart.transpose();

        kappaL = plate->v_bendingElement[i].kappa - plate->v_bendingElement[i].kappaBar;

        temp = - 1.0 / len * kappaL.transpose() * localEI;
