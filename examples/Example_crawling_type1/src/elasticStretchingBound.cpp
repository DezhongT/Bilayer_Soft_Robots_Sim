...
elasticStretchingBound::elasticStretchingBound(elasticPlate &m_plate, timeStepper &m_stepper)
{
	plate = &m_plate;
    stepper = &m_stepper;
...
elasticStretchingBound::~elasticStretchingBound()
{
	;
}

void elasticStretchingBound::computeFs()
{
	for (int k = 0; k < plate->v_coupleStretching.size(); k++)
	{
		flocal = VectorXd::Zero(6);

		int nv_1 = plate->v_coupleStretching[k].nv_1;
		int nv_2 = plate->v_coupleStretching[k].nv_2;
		...
		double epsX = edgeLen / plate->v_coupleStretching[k].refLength - 1.0;

		Vector3d f = EA * tangent * epsX;

		localDOF = plate->v_coupleStretching[k].arrayNum;

		//cout << localDOF.norm() << endl;
...
void elasticStretchingBound::computeJs()
{
	...
	for (int k = 0; k < plate->v_coupleStretching.size(); k++)
	{
		...
		double refLength = plate->v_coupleStretching[k].refLength;
...
void elasticStretchingBound::setFirstJacobian()
{
	for (int k = 0; k < plate->v_coupleStretching.size(); k++)
	{
		localDOF = plate->v_coupleStretching[k].arrayNum;
