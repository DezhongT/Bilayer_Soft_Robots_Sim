...
void elasticBendingForce::computeFb()
{
    ...
    for (int i = 0; i < plate->bendingNum; i++)
    {
        ...
        f = - plate->v_bendingElement[i].stiffness * relevantPart * EIMat * kappaL / plate->v_bendingElement[i].voroniLength;
...
void elasticBendingForce::computeJb()
{
	...
	for (int i = 0; i < plate->bendingNum; i++)
    {
        ...
        Jbb = (Jbb + temp(0) * DDkappa1 + temp(1) * DDkappa2) * plate->v_bendingElement[i].stiffness;
