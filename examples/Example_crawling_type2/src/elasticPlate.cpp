...
Vector3d elasticPlate::getVelocityOld(int i)
{
	...
	uCurrent(0) = u(3 * i + 0) / dt;
	uCurrent(1) = u(3 * i + 1) / dt;
	uCurrent(2) = u(3 * i + 2) / dt;
...
void elasticPlate::setupGeometry()
{
	...
	{
		Vector2i eCurrent = edge[i];

		cout << eCurrent.transpose() << endl;
	}
...
void elasticPlate::computeBending()
{
	...
	for (int i = 0; i < bending.size(); i++)
	{
		...
		*/
		...
		if (i >= 13 && i <=48 )
		{
			m_bendingElement.stiffness = 0.1;
		}
		else
		{
			m_bendingElement.stiffness = 1.0;
		}
